...
> - **初学者建议先用 CPU 实例。** 本课程大多数内容使用 CPU 实例即可完成。只有《4.1 用蒸馏让小模型掌握专业能力》和《4.2 部署模型》两个课程建议使用 GPU 实例；如果使用 GPU，需要按对应章节说明单独创建 GPU 实例，并在新实例中重新安装课程环境。GPU 实例消耗计算时更快，也更容易在额度用尽后产生较高费用。
...
4. 在新建实例页面，填写实例名称、选择资源规格和镜像：
    ...
    <img src="https://img.alicdn.com/imgextra/i1/O1CN01COwYFj1ERQtap4LW4_!!6000000000348-2-tps-2198-1388.png" width="500px">
      ...
      *   **资源规格**：优先选择**免费试用页签**中的**ecs.g6.xlarge**。如果页面没有“免费试用”页签、没有可用免费规格，或计费提示不是你预期的免费额度抵扣，请先不要创建实例，改用 ModelScope Notebook 或本地 IDE。
          ...
          > **初学者建议先选择 CPU 实例。** `ecs.g6.xlarge` 足以运行本课程的大多数项目。只有《4.1 用蒸馏让小模型掌握专业能力》和《4.2 部署模型》两个课程建议使用 GPU；需要使用 GPU 时，请按对应章节说明单独创建 GPU 实例，并在新实例中重新安装课程环境。
